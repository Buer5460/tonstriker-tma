          ...
          <div className="mt-6 border-t border-gray-700 pt-6 space-y-3">
            {scrapedMatches.map((m, i) => (
              <div key={i} className="flex justify-between items-center bg-gray-900 p-4 rounded-lg border border-gray-700">
                ...
                {/* 💡 修复完毕：现在传入了五个参数，包含 logoA 和 logoB */}
          ...
          return (
            <div key={matchName} className="bg-gray-800 p-6 rounded-xl border border-red-900/50 flex justify-between items-center">
              <div>
                ...
                <span className="text-yellow-400 font-bold text-sm bg-yellow-400/10 px-3 py-1 rounded inline-block mt-2">奖池累计: {totalAmount} $GOAL</span>
              </div>
              <div className="flex space-x-3">
                <button onClick={() => handleSettle(matchName, matchName.split(' vs ')[0])} className="bg-red-900 px-6 py-3 rounded-lg font-black text-sm">主胜派奖</button>
                <button onClick={() => handleSettle(matchName, matchName.split(' vs ')[1])} className="bg-blue-900 px-6 py-3 rounded-lg font-black text-sm">客胜派奖</button>
              </div>
            </div>
          );
        })}
